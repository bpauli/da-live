...
export default class DaListItem extends LitElement {
  ...
  // PROTO start - lazy load on expand, refetch every time, clear on collapse
  async _protoLoadStatus() {
    this._protoStatus = undefined;
    this.requestUpdate();
    const status = await protoFetchStatus({ path: this.path, ext: this.ext });
    this._protoStatus = status;
    this.requestUpdate();
  }
  // PROTO end
  ...
  toggleExpand() {
    ...
    if (this.classList.contains('is-expanded')) {
      this.updateAEMStatus();
      this.updateDAStatus();
      this._protoLoadStatus(); // PROTO
    } else {
      this._preview = null;
      this._live = null;
      this._version = null;
      this._lastModifedBy = null;
      this._protoStatus = null; // PROTO
    }
  }
